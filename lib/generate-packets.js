...
const wait = ms => new Promise((resolve, reject) => setTimeout(() => resolve('timeout'), ms))
...
async function triggerSetCooldown (server, bot, mcData) {
  ...
  const item = bot.inventory.slots.find(x => x?.name === 'ender_pearl')
  console.log(item)
  bot.equip(item, 'hand')
  bot.activateItem()
  await setCooldown
...
async function triggerAnimation (server, bot, mcData) {
  const commands = [
    ['/summon zombie', '/kill @e[type=zombie]'], // >= 1.11
    ['/summon Zombie', '/kill @e[type=Zombie]'] // < 1.11
  ]
  const command = mcData.isNewerOrEqualTo('1.11') ? commands[0] : commands[1]
  const animation = once(bot._client, 'animation')
  bot.chat('/gamemode survival')
  await bot.awaitMessage('nmptestbot: Set own game mode to Survival Mode')
  bot.chat(command[0])
  await animation
  bot.chat('/gamemode creative')
  bot.chat(command[1])
