...
async function runTests (server, bot) {
  let botEnded = false
  bot.on('end', () => { botEnded = true })
  for (const trigger of triggers) {
    if (botEnded) {
      console.log(`${trigger.name} skipped (bot disconnected)`)
      continue
    }
    try {
      ...
      console.log(`${trigger.name} errored: ${err.message}`)
    }
...
async function generatePackets (server, bot, mcData) {
  ...
  // Wait for additional packets, but stop early if bot disconnects
  await new Promise((resolve) => {
    const timeout = setTimeout(resolve, 60 * 1000)
    bot.on('end', () => {
...
async function triggerNamedEntitySpawn (server, bot, mcData) {
  ...
  botOne.on('error', () => {}) // suppress connection errors
  botOne._client.on('error', () => {}) // suppress connection errors
...
async function triggerFurnace (server, bot, mcData) {
  ...
  if (!block) {
    console.log('Could not find furnace block, skipping furnace test')
    return
  }
