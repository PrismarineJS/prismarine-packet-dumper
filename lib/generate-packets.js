...
async function triggerPainting (server, bot, mcData) {
  ...
  const wantedCommand = mcData.isNewerOrEqualTo('1.10') ? commands[0] : commands[1]
  ...
  const collect = once(bot._client, 'collect')
  bot.chat(wantedCommand)
  await Promise.all([entityMeta, spawn, painting, collect])
}
