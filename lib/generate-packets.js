...
const triggers = [triggerNamedEntitySpawn, triggerTabComplete, triggerChat, triggerRespawn, triggerGameStateChange, triggerStopSound]
...
async function triggerGameStateChange (server, bot) {
  ...
  await p1
  console.log('game_state_change')
}

async function triggerStopSound (server, bot) {
  const p2 = once(bot._client, 'stop_sound')
