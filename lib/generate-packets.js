...
async function triggerBlockBreakAnimation (server, bot) {
  ...
  bot.on('messagestr', msg => console.log(`client: ${msg}`))
  bot.chat(`/op ${botOne.username}`)
  await bot.awaitMessage('Opped tempBot', 'Made tempBot a server operator')
  if(require('minecraft-data')(bot.version).isNewerOrEqualTo('1.13')){
    bot.chat('/setblock ~ ~-1 ~ dirt replace')
  } else {
    bot.chat('/setblock ~ ~-1 ~ dirt')
  }
