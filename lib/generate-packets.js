...
async function triggerFurnace (server, bot, mcData) {
  const furnaceRecieved = once(bot.inventory, 'updateSlot')
  server.writeServer(`give ${bot.username} furnace\n`)
  await furnaceRecieved
  bot.chat('/setblock ~ ~1 ~1 furnace')
  await once(bot.world, 'blockUpdate')
  const block = await bot.findBlock({ matching: mcData.blockByName.furnace.id })
  console.log(block)
}
